...
정규화의 가장 큰 목표는 테이블 간의 **중복된 데이터를 허용하지 않는 것**이다. 중복된 데이터를 만들지 않으면 무결성을 유지할 수 있고, DB저장 또한 효율적으로 관리할 수 잇다.
...
### 제 1정규화 (1NF) - 테이블 컬럼이 원자 값(단일의 값) 을 갖도록 테이블을 분리
...
### 제 3정규화 (3NF) - 이행적 종속을 없애기 위해 테이블을 분리
...
아래의 테이블의 기본 키는 (`Tournament`, `Year`) 의 복합키 이다. `Winner` 컬럼은 이 복합키를 통해 결정된다. (2NF 만족)
