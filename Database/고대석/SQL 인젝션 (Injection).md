...
1. input 창에 다른 쿼리문을 함께 입력해 DB를 조작하는 공격 방법 ⇒ 인증 우회
2. 시스템에서 발생하는 에러 메시지를 이용해 공격하는 방법 ⇒ 데이터 노출
3. SQL문을 실행할 수 있는 객체이며 특수문자를 자동으로 파싱해주어 SQL Injection을 막을 수 있는 객체 ⇒ Prepared Statement